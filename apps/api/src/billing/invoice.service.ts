...
export class InvoiceService {
  ...
  async cancel(tenantId: string, id: string, userId: string, reason?: string) {
    const { result, invoiceNo, total } = await this.db.runInTenant(tenantId, async (m) => {
      ...
      // 'recorded' is an e-way bill generated by hand on the portal. It is just
      // as live as one this system transmitted, so it blocks a cancel the same way.
  ...
  async setTransport(tenantId: string, id: string, userId: string, dto: Record<string, unknown>) {
    const { result, invoiceNo, changes } = await this.db.runInTenant(tenantId, async (m) => {
      ...
      // The e-way bill number itself, for plants that generate it by hand on the
      // government portal — which is every plant until GST transmission is
      // switched on. Without somewhere to put it the number was simply lost: the
      // challan could never print it, and the compliance check counted every
      // invoice over ₹50,000 as "e-way bill pending" forever, even when one had
      // been generated for every load. An alert that is always red is one nobody
      // reads.
      if ('ewayBillNo' in dto) {
        // A bill obtained through the portal integration is the authoritative
        // record — it was transmitted and acknowledged. Never let a typed value
        // overwrite it; cancel it on the portal instead.
        if (invoice.ewayStatus === 'generated') {
          throw badReq(
            'This invoice already has an e-way bill generated through the portal. Cancel it on the portal rather than typing over it here.',
          );
        }
        const raw = dto.ewayBillNo;
        if (raw === null || String(raw).trim() === '') {
          patch.ewayBillNo = null;
          patch.ewayBillDate = null;
          patch.ewayValidUntil = null;
          patch.ewayStatus = 'not_generated';
          parts.push('cleared e-way bill');
        } else {
          const no = String(raw).replace(/\s+/g, '');
          // The portal issues a 12-digit number. Catching a mistyped one here is
          // the difference between a wrong number on the driver's document and a
          // correction while the vehicle is still on the yard.
          if (!/^\d{12}$/.test(no)) {
            throw badReq('An e-way bill number is the 12 digits shown on the portal, e.g. 231234567890');
          }
          patch.ewayBillNo = no;
          patch.ewayStatus = 'recorded';
          parts.push(`e-way bill ${no}`);
          if (dto.ewayBillDate !== undefined) {
            patch.ewayBillDate = dto.ewayBillDate ? new Date(String(dto.ewayBillDate)) : null;
          } else if (!invoice.ewayBillDate) {
            patch.ewayBillDate = new Date();
          }
          if (dto.ewayValidUntil !== undefined) {
            patch.ewayValidUntil = dto.ewayValidUntil ? new Date(String(dto.ewayValidUntil)) : null;
          }
        }
      }
      for (const [key, field] of [['ewayBillDate', 'ewayBillDate'], ['ewayValidUntil', 'ewayValidUntil']] as const) {
        // Allow the dates to be corrected on their own, once a number is on file.
        if (key in dto && !('ewayBillNo' in dto)) {
          if (invoice.ewayStatus === 'generated') {
            throw badReq('This invoice\'s e-way bill came from the portal — change it there, not here.');
          }
          if (!invoice.ewayBillNo) throw badReq('Record the e-way bill number before setting its dates');
          const v = dto[key];
          patch[field] = v ? new Date(String(v)) : null;
          parts.push(`e-way ${key === 'ewayBillDate' ? 'date' : 'validity'}`);
        }
      }
