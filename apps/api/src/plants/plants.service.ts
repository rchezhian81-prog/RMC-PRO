...
export class PlantsService extends TenantCrudService<Plant> {
  ...
  override async create(tenantId: string, dto: Record<string, unknown>): Promise<Plant> {
   ...
   */
  override async update(tenantId: string, id: string, dto: Record<string, unknown>): Promise<Plant> {
