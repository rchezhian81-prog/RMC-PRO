...
test('one company block, built once and drawn once, for all nine documents', () => {
  ...
  assert.equal((src.match(/drawCompanyHeader\(doc, data, left\)/g) ?? []).length, 9, 'invoice, receipt, challan, quotation, weighbridge, statement, purchase order, payment voucher, payment advice');
...
test('one signature block, drawn by one helper, on the six documents that close with it', () => {
  const src = codeOnly(readFileSync(resolve(repoRoot, 'apps/api/src/sales/pdf.service.ts'), 'utf8'));
  assert.equal((src.match(/function drawSignatoryBlock\(/g) ?? []).length, 1, 'one definition');
  assert.equal((src.match(/drawSignatoryBlock\(doc, data\.companyName, left, right\)/g) ?? []).length, 6, 'invoice, receipt, quotation, statement, purchase order, payment advice');
