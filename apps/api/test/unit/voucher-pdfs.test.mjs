...
test('every document is printable from the screen that lists it — one Print button per row, through openPdf', () => {
  ...
  // The viewer is an iframe on a blob: URL; the CSP must let frames load it,
  // or the PDF is a broken-document icon (seen before this line existed).
  const cfg = readFileSync(resolve(repoRoot, 'apps/web/next.config.mjs'), 'utf8');
  assert.match(cfg, /`frame-src 'self' blob:`/, 'CSP allows the viewer iframe to show a blob: PDF');
  assert.match(cfg, /`object-src 'none'`/, 'and still refuses plugins');
