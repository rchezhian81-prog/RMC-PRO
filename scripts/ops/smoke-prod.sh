...
req() {
  ...
  # curl's -w '%{http_code}' already prints 000 when it can't connect, so no
  # fallback echo here (that would double it, e.g. "000000").
...
import sys, json
try:
    d = json.load(open(__import__("os").environ["SMOKE_BODY"]))
except Exception:
    d = None
...
login() { # login EMAIL PASSWORD -> echoes access_token ("" on failure)
  local code; code="$(req POST /auth/login '' "$(mkjson login "$1" password "$2")")"
  { [ "$code" = "200" ] || [ "$code" = "201" ]; } && jget data.access_token || echo ""
}
...
# ---- 0. Reachability ------------------------------------------------------
eq "$(req GET /health)" 200 "API /health is reachable"

# ---- 1. Owner: everything still works (owner bypasses every gate) ----------
echo "-- owner (should see everything) --"
OT="$(login "$OWNER_LOGIN" "$OWNER_PASSWORD")"
present "$OT" "owner logs in"
[ -n "$OT" ] || { echo "cannot continue without an owner token"; echo; echo "SMOKE: $pass passed, $fail failed"; exit 1; }
...
if [ -n "${NONADMIN_LOGIN:-}" ] && [ -n "${NONADMIN_PASSWORD:-}" ]; then
  echo "-- low-privilege user (should be gated OUT; use a batching/dispatch/store operator) --"
  echo "   NOTE: if this account happens to hold reports.view/ai.use, these will 'fail' — that is not a regression, pick a narrower account."
  NT="$(login "$NONADMIN_LOGIN" "$NONADMIN_PASSWORD")"
  present "$NT" "low-priv user logs in"
  if [ -n "$NT" ]; then
